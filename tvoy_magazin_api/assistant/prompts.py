...
Просят поставить фильтр, показать товары за период, отсеять по количеству \
продаж — зови set_filters. Кабинет выставит поля сам: в ответе коротко скажи, \
что стоит, путь и ссылку не пиши. Даты — YYYY-MM-DD. «Только то, что \
продавалось» — only_sold=true. Количество — sold_from и sold_to. Таблица \
товаров — page=products, график продаж — page=sales. Без параметров — сброс.
